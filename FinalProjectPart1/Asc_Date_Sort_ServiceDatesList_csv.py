...
with open ("csv_inputs/ServiceDatesList.csv", 'r') as csv_servicedates:
    serviceDate_list_csv = csv.reader (csv_servicedates)
    next(serviceDate_list_csv) #skips header row in csv file "item_id, service_date"
    for i in serviceDate_list_csv:
        service_date_sorted.append (i)  # extracts and appends position 1 (service_Date) for each row of csv data [item_id (0), service_date (1)] into service_date_sorted array,

    for i in service_date_sorted:
        i[1] = datetime.strptime(i[1], "%m/%d/%Y").date()
    
        # service_date_sorted = [i for i in service_date_sorted ] 
        # service_date_sorted = [i[1], datetime.strptime(i[1], "%m/%d/%Y").date() for i in service_date_sorted ] 
    ...
    service_date_sorted.sort (key=lambda x:x[1]) #dates sorted asc
